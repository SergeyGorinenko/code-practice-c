...
void TwoSum::test() {
    ...
    int target1 = 9;
    std::vector<int> result1 = instance.twoSum(nums1, target1);
    std::cout << "{" + joinVector(nums1, ", ") + "}: {" + joinVector(result1, ", ") + "}" << std::endl;
    
    // Test 2
    std::vector<int> nums2 = {3,2,4};
    int target2 = 6;
    std::vector<int> result2 = instance.twoSum(nums2, target2);
    std::cout << "{" + joinVector(nums2, ", ") + "}: {" + joinVector(result2, ", ") + "}" << std::endl;

    // Test 3
    std::vector<int> nums3 = {3,3};
    int target3 = 6;
    std::vector<int> result3 = instance.twoSum(nums3, target3);
